...
export function TopBar({ board, toggleTaskModule }){
  const {appState, setAppState, toggleSideMenu, selectedBoard} = useContext(AppState)
  ...
  const toggleEditBoardModal = () => {
    // prevent function is no board is selected
    if(appState.activeBoard === null) return

    setAppState(prevState => (
      {
        ...prevState,
        editBoardModal: !appState.editBoardModal
      }
    ))
    handleClick()
  }
  ...
  const toggleDeleteBoardModal = () => {
    ...
    setAppState(prevState => (
      {
        ...prevState,
        deleteBoardModal: !appState.deleteBoardModal
      }
    ))
    handleClick()
  }
  ...
  function EditBoard(){
    return(
      <ul className="edit-board board-options">
        <li onClick={toggleEditBoardModal}>Edit Board</li>
        <li onClick={toggleDeleteBoardModal}>Delete Board</li>
      </ul>
    )
  }
  ...
  return(
    <nav className="nav">
      {appState.showSideMenu === false ?  <picture onClick={toggleSideMenu} id="nav__logo" className="nav__logo"> <svg width="153" height="26" viewBox="0 0 153 26" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <div className="nav__menu">
       {isLargerScreen ? <svg className="nav__logo-mobile" width="24" height="25" viewBox="0 0 24 25" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      <svg onClick={showBoards} className="nav__arrow" width="9" height="7" viewBox="0 0 9 7" fill="none" xmlns="http://www.w3.org/2000/svg">
        <path d="M1 1L5 5L9 1" stroke="#635FC7" stroke-width="2" />
      </svg>
    </div>
    <div className="nav__buttons">
      <button onClick={toggleTaskModule} className="primary-btn add-task-btn"><svg width="12" height="12" viewBox="0 0 12 12" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M7.368 12V7.344H12V4.632H7.368V0H4.656V4.632H0V7.344H4.656V12H7.368Z" fill="white" />
        </svg></button>
      <svg onClick={handleClick} className="edit-task-btn" width="4" height="16" viewBox="0 0 4 16" fill="none" xmlns="http://www.w3.org/2000/svg">
